...
export const ProjectCard = ({ project }: { project: ProjectCardProps }) => {
    ...
    const Icon = ({ ...props }) => {
        return showMore ? <ChevronUp {...props} /> : <ChevronDown {...props} />;
    }
    ...
    return (
        <div ref={ref} className={`relative max-w-full h-24 ${showMore && 'h-fit'} text-pine-tree-green dark:text-corn-silk/70 flex flex-col items-start md:mx-auto ${inView ? 'animate__animated animate__fadeIn' : ''}`}>
            <div className="font-display font-medium text-start text-2xl">{project.title}</div>
            ...
            <Icon
                ...
                className="absolute top-2 right-5 block cursor-pointer hover:opacity-80" />

            {showMore && <div className="flex flex-col gap-y-2">
                ...
                <div className="relative flex">
                    <img src={project.projectImages[0]} alt="ecommerce_image" className="w-full max-w-[425px]" />
                    <div className="hidden min-[425px]:flex flex-col">
                        {project.projectImages.map((img, idx) =>
                            <img src={img} key={idx} className="max-w-" />
                        )}
                    </div>
                ...
                <div className="w-full h-8 min-h-8 mt-auto rounded-md bg-woodland-green text-corn-silk flex justify-center hover:opacity-80 hover:cursor-pointer transition-all duration-300">
